...
The unpatched conventional control (Picard without `IS_BISULFITE_SEQUENCE`) is produced per cell
by both workflows and is reported in the CI briefings as a negative control: graft recall and PPV
collapse to ~0 percentage points while host specificity is unaffected, which is the evidence that
the bisulfite NM recalculation is load-bearing. It has no recorded per-cell expectation, so its
briefing rows carry measured values and are marked as outside the recorded contract rather than
compared against it, and it remains excluded from the Table S6 and S7 numbers — those report the
two patched arms only.
