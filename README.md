...
One graft/host pair, classified end to end — 103,025 reads, split by the evidence each mate carries:

```text
$ xenofilx run --graft graft.bam --host host.bam --output out

=== Sample Results ===
Sample                  Total GraftOnly HostOnly     Both     Graft(%)      Host(%)   Discard(%) TotalGraft(%)   Thresh
-----------------------------------------------------------------------------------------------
graft.queryname_Filtered.bam   103025    89868    13125       32        0.07%       12.74%       87.19%        0.07%        4
```

`GraftOnly` and `HostOnly` are the reads whose evidence points one way, `Both` the ones that carry
both, and `Discard(%)` the fraction removed. The threshold column shows the cutoff that was applied.
...
```bash
git clone https://github.com/otterlab-bio/xenofilx.git
cd xenofilx
...
MIT · [otterlab-bio/xenofilx](https://github.com/otterlab-bio/xenofilx)
