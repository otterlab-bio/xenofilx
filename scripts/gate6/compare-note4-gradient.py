...
# The unpatched control: Picard SetNmMdAndUqTags with IS_BISULFITE_SEQUENCE omitted, then
# XenofilteR. It is a negative control, not a reported arm: graft recall collapses to ~0
# percentage points while host specificity is unaffected, which is what shows the bisulfite NM
# recalculation is load-bearing. It has no recorded per-cell expectation, so these metrics are
# carried as observed values and never classified.
CONVENTIONAL_TOOL = "conventional_picard_xenofilter"

METRIC_NAMES = {
    "modern_recall": (MODERN_TOOL, "sensitivity_graft_recall"),
    "legacy_recall": (LEGACY_TOOL, "sensitivity_graft_recall"),
    "conventional_recall": (CONVENTIONAL_TOOL, "sensitivity_graft_recall"),
    "modern_spec": (MODERN_TOOL, "specificity_host_recall"),
    "legacy_spec": (LEGACY_TOOL, "specificity_host_recall"),
    "conventional_spec": (CONVENTIONAL_TOOL, "specificity_host_recall"),
    "modern_ppv": (MODERN_TOOL, "precision_graft_ppv"),
    "legacy_ppv": (LEGACY_TOOL, "precision_graft_ppv"),
    "conventional_ppv": (CONVENTIONAL_TOOL, "precision_graft_ppv"),
}
